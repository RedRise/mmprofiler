...
def test_push_order():
    ...
    offersLists.add_maker_order(copy.copy(offer))
    assert not offersLists.has_bid()
    assert offersLists.has_ask()
    assert offer == offersLists.get_best_ask()

    bid = Order(OrderType.BUY, 100, 1)
    offersLists.add_maker_order(copy.copy(bid))
    assert bid == offersLists.get_best_bid()


def test_append_best_bid():
    offersLists = OffersLists()
    offersLists.add_maker_order(Order(OrderType.BUY, 100, 2))
    assert offersLists.get_best_bid().price == 100

    offersLists.add_maker_order(Order(OrderType.BUY, 110, 1))
    assert offersLists.get_best_bid().price == 110


def test_append_best_ask():
    offersLists = OffersLists()
    offersLists.add_maker_order(Order(OrderType.SELL, 110, 2))
    assert offersLists.get_best_ask().price == 110

    offersLists.add_maker_order(Order(OrderType.SELL, 100, 1))
...
def test_add_worst_bid():
    ...
    offersLists.add_maker_order(Order(OrderType.BUY, 90, 1))
    assert offersLists.get_best_bid().price == 100
...
def test_add_worst_ask():
    ...
    offersLists.add_maker_order(Order(OrderType.SELL, 120, 1))
    assert offersLists.get_best_ask().price == 110
